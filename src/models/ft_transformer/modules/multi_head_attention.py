...
class MultiheadAttention(nn.Module):
    ...
    **Examples**
    ...
    def forward(self, x_q: Tensor, x_kv: Tensor, attention_probs_reversed=None) -> Tuple[Tensor, Tensor] | Tuple[Tensor, Tensor, Tensor]:
        ...
        if not attention_probs_reversed is None:
            x = attention_probs_reversed @ x
        ...
        ## attentionの可視化用実装
        # x_squeezed = attention_probs.reshape(batch_size, self._n_heads, attention_probs.shape[1], attention_probs.shape[2])
        ...
        if self.use_weights:
            return (
                ...
                # torch.tensor([]),
                attention_probs,
                attention_probs.reshape(batch_size, self._n_heads, attention_probs.shape[1], attention_probs.shape[2]).detach().to("cpu"),
            )
        else:
            return x, attention_probs.reshape(batch_size, self._n_heads, attention_probs.shape[1], attention_probs.shape[2]).detach().to("cpu")
