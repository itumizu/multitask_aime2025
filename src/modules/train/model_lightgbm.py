...
def trainLightGBM(
    ...
    # Model
    model = selectModel(config=config, gpus=gpus, fold_test_num=0, fold_val_num=0)
    ...
    model.fit(
        X=X_train,
        y=y_train,
        eval_metric=config.args.models.params.metric,
        eval_set=[(X_train, y_train), (X_val, y_val)],
        callbacks=[
            lightgbm.early_stopping(stopping_rounds=config.args.callbacks.early_stopping.patience, verbose=True),
    ...
    y_pred_train = model.predict_proba(X=X_train)[:, 1]

    # Evaluate on validation data
    y_pred_val = model.predict_proba(X=X_val)[:, 1]
    ...
    for idx, target_name in enumerate(config.args.tasks.targets):
        train_loss = evals_result["training"][config.args.models.params.metric]
        val_loss = evals_result["valid_1"][config.args.models.params.metric]
    ...
    if not return_validation_results_only:
        for idx, target_name in enumerate(config.args.tasks.targets):
            y_pred_test = model.predict_proba(X=X_test)[:, 1]
